...
public class Utils
{
    ...
    GetOffsetVector(int y, int x, int keypoint, Tensor offsets)
    {
        // Get the offset values for the provided heatmap coordinates
        return new Vector2(offsets[0, y, x, keypoint + 17],
    ...
    /// <returns></returns>
    public static Vector2
    GetImageCoords(Keypoint part, int stride, Tensor offsets)
    {
    ...
    static Vector2Int
    GetStridedIndexNearPoint(Vector2 point, int stride, int height, int width)
    {
    ...
    static Vector2
    GetDisplacement(int edgeId, Vector2Int point, Tensor displacements)
    {
        ...
        return new Vector2(displacements[0,
            point.y,
            point.x,
            numEdges + edgeId],
    ...
    )
    {
        ...
        Vector2Int sourceKeypointIndices =
            GetStridedIndexNearPoint(sourceKeypoint.position,
            stride,
            height,
            width);
        ...
        Vector2 offsetVector =
            GetOffsetVector(displacedPointIndices.y,
            displacedPointIndices.x,
            targetKeypointId,
            offsets);
    ...
    )
    {
        ...
        instanceKeypoints[root.id] =
            new Keypoint(root.score, rootPoint, root.id);
        ...
        for (int edge = numEdges - 1; edge >= 0; --edge)
        {
            ...
            if (
                instanceKeypoints[sourceKeypointId].score > 0.0f &&
                instanceKeypoints[targetKeypointId].score == 0.0f
            )
            {
                instanceKeypoints[targetKeypointId] =
                    TraverseToTargetKeypoint(edge,
    ...
    )
    {
        ...
        return poses
            .Any(pose =>
                (vec - pose[keypointId].position).sqrMagnitude <=
                squaredNmsRadius);
    }
    ...
    )
    {
        ...
        int yEnd =
            Mathf.Min(heatmapY + localMaximumRadius + 1, heatmaps.height);
        ...
        for (int yCurrent = yStart; yCurrent < yEnd; ++yCurrent)
        {
            ...
            int xEnd =
                Mathf.Min(heatmapX + localMaximumRadius + 1, heatmaps.width);
    ...
    BuildPartList(float scoreThreshold, int localMaximumRadius, Tensor heatmaps)
    {
    ...
    )
    {
        ...
        List<Keypoint> list =
            BuildPartList(scoreThreshold, kLocalMaximumRadius, heatmaps);
        ...
        while (poses.Count < maxPoseDetections && list.Count > 0)
        {
            ...
            if (
                WithinNmsRadiusOfCorrespondingPoint(poses,
                squaredNmsRadius,
                rootImageCoords,
                root.id)
            )
