...
public class PoseEstimator : MonoBehaviour
{
	...
	[
			Tooltip(
					"The ComputeShader that will perform the model-specific preprocessing")
	]
	...
	private void InitializeCamera()
	{
		...
		mainCamera.transform.position =
				new Vector3(videoDims.x / 2, videoDims.y / 2, -10f);
	...
	void Start()
	{
		if (useWebcam)
		{
			...
			Debug.Log("videoDims: " + videoDims.x);
		}
		else
		{
			// Update the videoDims.y
	...
	private void ProcessImageGPU(RenderTexture image, string functionName)
	{
		...
		posenetShader
				...
				result.width / numthreads,
				result.height / numthreads,
				1);
	...
	private void ProcessOutput(IWorker engine)
	{
		...
		else
		{
			...
			poses =
					Utils
							.DecodeMultiplePoses(heatmaps,
							offsets,
							displacementFWD,
							displacementBWD,
							stride: stride,
							maxPoseDetections: maxPoses,
