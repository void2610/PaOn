...
public class Calibration : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        loadText =
            (
            Resources
                .Load("CalibrationResources/CalibrationMessage",
                typeof (TextAsset)) as
            TextAsset
            ).text;
        ...
        if (pose[0] != null)
        {
            switch (state)
            {
                case Phase.PoseEstimation:
                    ...
                    if (
                        pose[0].score > minConfidence &&
                        pose[15].score > minConfidence &&
                        pose[16].score > minConfidence
                    )
                    {
                ...
                case Phase.HandEstimation:
                    Debug.Log("Phase2");
                    if (!isRunning) message.text = splitText[1];
