...
namespace OpenPose.Example
{
    ...
    public class OpenPoseUserScript : MonoBehaviour
    {
        ...
        public void SetFaceEnabled(bool enabled)
        {
            faceEnabled = enabled;
        }

        public void SetRenderThreshold(string s)
        {
            float res;
        ...
        public void SetPoseResX(string s)
        {
            int res;
            if (int.TryParse(s, out res))
            {
        ...
        public void SetPoseResY(string s)
        {
            int res;
            if (int.TryParse(s, out res))
            {
        ...
        public void SetHandResX(string s)
        {
            int res;
            if (int.TryParse(s, out res))
            {
                handResolution.x = res;
            }
        }

        public void SetHandResY(string s)
        {
            int res;
            if (int.TryParse(s, out res))
            {
                handResolution.y = res;
            }
        }

        public void SetFaceResX(string s)
        {
            int res;
            if (int.TryParse(s, out res))
            {
                faceResolution.x = res;
            }
        }

        public void SetFaceResY(string s)
        {
            int res;
            if (int.TryParse(s, out res))
            {
                faceResolution.y = res;
            }
        }
        ...
        {
            OPWrapper
                .OPConfigurePose(/* poseMode */
                PoseMode.Enabled, /* netInputSize */
                netResolution, /* outputSize */
                null,
                /* keypointScaleMode */
                ...
                null,
                /* heatMapTypes */
                HeatMapType.None, /* heatMapScaleMode */
                ScaleMode.UnsignedChar,
                /* addPartCandidates */
                false, /* renderThreshold */
                renderThreshold, /* numberPeopleMax */
                maxPeople,
                /* maximizePositives */
                false, /* fpsMax fps_max */
                -1.0,
                /* protoTxtPath */
                "", /* caffeModelPath */
                "", /* upsamplingRatio */
            ...
            OPWrapper
                .OPConfigureHand(/* enable */
                handEnabled, /* detector */
                Detector.Body, /* netInputSize */
                handResolution,
                /* scalesNumber */
                1, /* scaleRange */
                0.4f, /* renderMode */
                ...
                /* alphaKeypoint */
                0.6f, /* alphaHeatMap */
                0.7f, /* renderThreshold */
                0.2f);

            OPWrapper
                .OPConfigureFace(/* enable */
                faceEnabled, /* detector */
                Detector.Body,
                /* netInputSize */
                faceResolution, /* renderMode */
                RenderMode.Cpu,
                /* alphaKeypoint */
                0.6f, /* alphaHeatMap */
                0.7f, /* renderThreshold */
            ...
            OPWrapper
                .OPConfigureExtra(/* reconstruct3d */
                false, /* minViews3d */
                -1, /* identification */
                false, /* tracking */
                -1,
            ...
            OPWrapper
                .OPConfigureInput(/* producerType */
                inputType, /* producerString */
                producerString,
                /* frameFirst */
                0, /* frameStep */
                1, /* frameLast */
            ...
            OPWrapper
                .OPConfigureGui(/* displayMode */
                DisplayMode.NoDisplay, /* guiVerbose */
                false, /* fullScreen */
                false);

            OPWrapper
                .OPConfigureDebugging(/* loggingLevel */
                Priority.High, /* disableMultiThread */
                false, /* profileSpeed */
                1000);
        }
        ...
        private IEnumerator UserRebootOpenPoseCoroutine()
        {
            ...
            yield return new WaitUntil(() =>
                    {
                        return OPWrapper.state == OPState.Ready;
                    });
        ...
        private void Update()
        {
            ...
            if (OPWrapper.OPGetOutput(out datum))
            {
                ...
                foreach (var
                    human
                    in
                    humanContainer.GetComponentsInChildren<HumanController2D>()
                )
                {
                ...
                if (frameCounter >= queueMaxCount || frameTimeQueue.Count <= 5)
                {
                    ...
                    frameCounter = 0;
                    avgFrameRate =
                        frameTimeQueue.Count /
                        (Time.time - frameTimeQueue.Peek());
